...
mod tests {
    ...
    macro_rules! seg_text {
        ($t:expr) => {
            MessageSegment::Text { text: $t.into() }
        };
    }
    ...
    fn t3_6_encode_custom_node() -> anyhow::Result<()> {
        ...
        let data = payload["custom"]["data"]
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("custom.data should be a string"))?;
    ...
    fn t3_10_decode_image() -> anyhow::Result<()> {
        ...
        let decoded = decode_message(&payload)?;
        assert!(
            matches!(decoded, MessageSegment::Image { ref image_url, width: 800, height: 600 }
    ...
    fn t3_11_decode_audio() -> anyhow::Result<()> {
        ...
        let decoded = decode_message(&payload)?;
        assert!(
            matches!(decoded, MessageSegment::Audio { ref audio_url, duration_ms: 5000 }
            if audio_url == "a.mp3")
        );
        Ok(())
    }
    ...
    fn t3_15_decode_custom_101() -> anyhow::Result<()> {
        ...
        let data = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, inner_str);
        let payload = make_ct_payload(
            101,
            serde_json::json!({
                "custom": {"type": 2, "data": data}
    ...
    fn t3_40_decode_empty_custom_data() -> anyhow::Result<()> {
        ...
        assert!(
            result.is_err(),
            "empty base64 custom data should produce an error"
        );
        Ok(())
    }
