...
pub fn generate_sign(t: &str, token: &str, data: &str) -> Result<String> {
    Ok(format!("{:x}", md5::compute(format!("{token}&{t}&{APP_KEY}&{data}"))))
}

/// Message ID for WebSocket frames: {rand_0..999}{timestamp_ms} 0
pub fn generate_mid() -> String {
    format!("{}{} 0", rand::thread_rng().gen_range(0..1000u32), epoch_ms())
}
...
mod tests {
    ...
    fn t3_22b_generate_sign_cross_verify() -> anyhow::Result<()> {
        ...
        assert_eq!(generate_sign("1700000000000", "abc123", "{}")?, "0de974d31357bb908f6e33b5c404f0b9");
        assert_eq!(generate_sign("t", "token", "data")?, "4d63fc0d3ae96eb57b92cb12637e6269");
        assert_eq!(generate_sign("", "", "")?, "4d04f571711c7683d0c5b45abfdc8ccb");
        Ok(())
    }
